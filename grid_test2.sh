...
# Log file path
log_file="compare_logs/wanda_new6_50_steps_layers_2.log"
...
for layer_name in "${layer_names[@]}"; do
    for weight_power in $(seq $weight_power_start $step $weight_power_end); do
        ...
        echo "Running test: weight_power=$weight_power, wanda_power=1.0, layer_name=$layer_name" | tee -a $log_file
    ...
    for wanda_power in $(seq $wanda_power_start $step $wanda_power_end); do
        # Log the current combination
        echo "Running test: weight_power=1.0, wanda_power=$wanda_power, layer_name=$layer_name" | tee -a $log_file
